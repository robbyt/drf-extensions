...
class _UsersConstructor(DefaultKeyConstructor):
    param_bit = UserKeyBit()
...
class HelloUserView(views.APIView):
    """ only allow authenticated users, to test the UserKeyBit
    """
    permission_classes = (IsAuthenticated,)

    @cache_response(key_func=_UsersConstructor())
    def get(self, request, *args, **kwargs):
